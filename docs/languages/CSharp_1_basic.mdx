...
引用类型与值类型不同。引用类型分两部分，一个是部分是存储真正数据的部分，是开辟在“托管堆”上的，另一个是指向该数据块的“句柄”，是在内存栈或其他任何位置的。我们创建引用类型的对象时，需要通过 `new` 关键字来在内存堆上开辟数据空间（不过又时可以省略）。例如字符串类型（`string`）就是一个引用类型：

```c#
string s1 = new string("Hello, world");	// s1 是一个句柄，指向一个 string 对象，该对象为 "Hello, world".
string s2 = "Hello, world";		// string s2 = new string("Hello, world"); 的简略写法
string s3;		// 仅仅定义了一个 string 句柄，并没有指向任何 string 对象！
```

没有指向任何对象的句柄的值是默认值 `null`，即上述代码等价于 `string s3 = null;`。

引用类型有很多，例如内置的引用类型 `object`、`string`、`dynamic`，数组类型、类类型（`class`）、委托类型（`delegate`）、接口类型（`interface`）、记录类型（`record`，C# 9.0）  

##### `object` 初探

`object` 是一个 C# 关键字，表示 .NET 类型 `System.Object` 类，即是一个类类型，它是所有类型（除指针类型）的公共基类。而它是个引用类型，这也意味着 `object` 的句柄可以指向所有类型的托管对象（关于托管的概念在后文中会有提到）。  

###### 装箱和拆箱

我们知道，值类型是没有句柄的。那么，一个 `object` 句柄是如何指向值类型的呢？这个机制就叫“装箱（Boxing）”和“取消装箱（Unboxing）”（也叫“拆箱”）。  
...
上面，`string` 是引用类型，因此必须使用 `new` 关键字在托管堆上开辟空间才真正创建了 `string` 对象（这个创建对象的过程通常称为“实例化”）。例如上面的 `s3`，即实例化了一个字符串对象，内容是空串，`s3` 是指向这个对象的句柄；而对于 `s2`，编译器会在编译时自动补上 `new` 关键字，其含义和 `s3` 相同；  

而对于 `s1`，我们并没有使用 `new` 关键字创建 `string` 对象，因此我们仅仅定义了一个句柄 `s1`，它不指向任何的对象，即它的值为 `null`。  

而对于值类型，例如 `int` 来说，虽然我们使用了 `new` 关键字来定义 `y`，但是并不会在托管堆上开辟一个内存空间，`y` 也不是句柄，而是一个真正的 `int` 变量，与 `x` 完全相同。  
...
```c#
var x = 4;	// x 是 int 类型
var s = "Hello, world";	// s 是 string 类型的句柄
var o = new object();	//  o 是 object 类型的句柄
```
...
值类型进行赋值是将该值类型的对象进行复制，而引用类型的复制是复制句柄，而非对象本身。  
...
+ `?` 常用于检查一个句柄是否为 `null`，若不为 `null` 则对其进行访问：  
...
此处 `arr` 具有两个元素，每个元素都是一个 `int[]` 的句柄。但是由于并没有用 `new` 为每个元素创建托管对象，因此每个句柄都是 `null`，并没有指向任何数组。·  
...
可以看到，这个 `Person` 类含有两个字段，一个是 `int` 类型的字段，一个是`string` 类型的句柄作为字段。  
...
方法的参数默认才用值传递，即将实参复制一份给形参。对于值类型来说，复制的是值类型的所有数据，对于引用类型来说，复制的是一个句柄。
...
此外，参数可以改成按引用方式传递，需要加上关键字 `ref`。如果加上关键字 `ref`，则值类型的形参和实参指代的是同一个值类型对象，而引用类型的实参和形参指代的是同一个句柄，例如上述代码改成：  
...
在每个类的方法中，可以访问到指向本对象自身的句柄，叫做 `this`。例子将在下面看到。  
...
class WebPage
{
    ...
    {
        this.url = url;		// 形参 url 覆盖了字段 url，需要用 this 句柄访问字段
    }
    private void Test()
    {
...
class WebPage
{
    ...
    public WebPage(string url)
    {
        Url = url;     // 形参 url 覆盖了字段 url，需要用 this 句柄访问字段
    }
    private void Test()
    {
...
// Dog dog = new Dog("");
// Animal ani2 = dog;  // 用 ani2 句柄指向 dog 指向的对象
// object o = 1;   // 将整数 1 装箱
...
显然，第一个参数常用于传递引发事件的对象，因此通常传递 `this` 句柄，而第二个参数则通常传递事件发生需要传递的参数。例如下面的例子：  
...
之前提到，引用类型的句柄可以为 `null`，而 C# 还支持可以为 `null` 的值类型 。可以为 `null` 的值类型。可以为 `null` 的值类型非常简单，只需要在对应的值类型后加上 `?` 即可构成可以为 `null` 的值类型，例如 `int?`、`double?`，等等。  
...
需要注意的是，与可为空的值类型不同，可为空的值类型与对应的不可为空值类型并不是同一个类型（值类型 `T` 对应的可为空的值类型是 `System.Nullable<T>`），而可为空的引用类型与不可为空引用类型是同一个类型，它们两个的区别只是在编译期间可以帮助检查是否忘记 `null` 的判断以防止直接对 `null` 句柄解引用，减少 BUG 的发生，而在程序编译结束后，即在运行期间两个类型并无任何区别。  
