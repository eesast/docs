...
在GitHub上操作，得到自己fork后的仓库链接`<yxj.url>`
...
如果你在`checkout -b`之后一直把改代码这件事情仍在一遍，那
...
`(modify) git commit -m"solve conflict"`
...
## 等待开发者merge
如果PR被拒绝，你可以继续更改代码commit、push、提PR直到merge；也可以直接弃坑lol
...
确认删除可以使用`(master) git branch -D modify`
