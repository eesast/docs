...
![multifile](assets/git/multifile.jpg)
...
![linear](assets/git/gitlinear.jpg)
...
4. 变基：
   ...
   ![gitrebase](assets/git/gitrebase.jpg)
...
![conflict](assets/git/conflict.jpg)
...
>
> ![vscconf](assets/git/vscconf.jpg)
>
> ![vsconf](assets/git/vsconf.jpg)
...
与 `.gitignore` 和 `.gitattributes` 文件不同，`.gitkeep` 文件不是一个 Git 内建支持的文件，只是开源社区约定俗成的文件。注意到，Git 通常只会追踪文件，而不会追踪空目录。但很多时候，我们需要创建一些空目录来进行占位或达成其他的目的。为了让 Git 也能够追踪这些目录，常见的做法是在这些目录里创建一个名为 `.gitkeep` 的空文件。这样，当 Git 会因为记录 `.gitkeep` 文件而将该目录也记录下来。

### 小结

到这里，相信你已经掌握了 Git 工具的基本使用。当然，在使用 Git 的过程中难免会遇到各种各样的问题、各种各样的~~玄学~~冲突。每当遇到这种情况，我们的解决方案正如下面这张著名的图：

![giterror](assets/git/giterror.png)
