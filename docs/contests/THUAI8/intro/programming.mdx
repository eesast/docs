...
![image-20230416010705076](../../assets/THUAI8/intro/image-20230416010705076.png)
...
![image-20230416010816392](../../assets/THUAI8/intro/image-20230416010816392.png)
...
`std::shared_ptr` 是 C++11 起提供的一种智能指针，其使用方法与裸指针类似，同样支持 `*` 和 `->` 运算符，例如：

```cpp
void print_int(std::shared_ptr<int> sp) {
...
void print_coord(std::shared_ptr<Point> sp) {
    std::cout << sp->x << ' ' << sp->y << std::endl;
}
```

关于智能指针的更多内容，请阅读 [C++ 相关小知识](../faq/cpptips.md#智能指针) 一节。
...
更多用法参见（点击进入）：[`{fmt}` documentation](https://fmt.dev/latest/index.html)。
